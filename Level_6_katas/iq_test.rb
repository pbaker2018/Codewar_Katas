...
# one number usually differs from the others in evenness. Help Bob — to check his answers,
...
def iq_test(numbers)
  array = numbers.split
  even = array.map{|e|e.to_i}.count{|e|e%2==0}
  odd = array.map{|e|e.to_i}.count{|e|e%2!=0}

  if even > odd
    index = array.map{|e|e.to_i}.index{|e|e.odd?}
    iq_index = index+1
    iq_index

  elsif odd > even
    index = array.map{|e|e.to_i}.index{|e|e.even?}
    iq_index = index+1
    iq_index
  end
end
